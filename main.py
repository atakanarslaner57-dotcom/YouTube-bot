...
def create_4k_video():
    print(">>> 4K Prodüksiyon Hattı Başlatıldı...")
    ...
    # 2. Karakterleri 4K Kalitesinde Kodla Yarat (Dosya hatasını önler)
    ...
    # Tırnak hatasını önlemek için komutu parçalara ayırdık
    filter_complex = (
        "[1:v]format=rgba,scale=600:-1[t];"
        "[2:v]format=rgba,scale=500:-1[p];"
        "[3:v]format=rgba,scale=400:-1[f];"
        "[0:v][t]overlay=x='500+100*sin(t)':y='1500+30*cos(t)'[bg1];"
        "[bg1][p]overlay=x='1800+150*cos(t)':y='800+60*sin(t)'[bg2];"
        "[bg2][f]overlay=x='3000+200*sin(t)':y='1600+100*cos(t)'"
    )

    cmd = [
        "ffmpeg", "-y",
        "-f", "lavfi", "-i", "color=c=0x000033:s=3840x2160:d=10:r=60", # Arka Plan
        "-i", "tori.png", "-i", "papi.png", "-i", "fini.png", "-i", "voice.mp3",
        "-filter_complex", filter_complex,
        "-c:v", "libx264", "-preset", "ultrafast", "-pix_fmt", "yuv420p", 
        "-c:a", "aac", "-shortest", "output_4k.mp4"
    ]

    subprocess.run(cmd)
    print(">>> 4K VIDEO TAMAMLANDI: output_4k.mp4")
